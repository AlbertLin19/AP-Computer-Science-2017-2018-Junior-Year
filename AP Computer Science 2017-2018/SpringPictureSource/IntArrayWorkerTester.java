...
int largest = worker.getLargest();
  ...
  /** Method to test shrinkArray */
//public static void testShrinkArray()
//{
//  IntArrayWorker worker = new IntArrayWorker();
//  int [][] nums2 = {{2, 4, 20, 15}, {4, 2, 10, 15}, {3, 3, 5, 10}, {7, 3, 2, 15}};
//  for (int[] row : nums2) {
//	  for (int spot : row) {
...
//  System.out.println();
//  worker.setMatrix(nums2);
//  int [][] results = worker.shrinkArray();
//  for (int[] row : results) {
//	  for (int spot : row) {
//		  System.out.print(spot + " ");
//	  }
...
//}
  ...
  public static void testShrinkArray() {
	  IntArrayWorker worker = new IntArrayWorker();
	  int [][] array = {{3, 5, 20, 15},{4, 4, 10, 15},{3, 3, 5, 10}, {7, 3, 2, 15}};
	  worker.setMatrix(array);
	  int[][] shrinkArray = worker.getShrinkArray();
	  for (int [] row : shrinkArray) {
		  for (int x : row) {
			  System.out.print(x + " ");
		  }
		  System.out.println();
